...
class WordCNN:
    def __init__(self, 
        ...
        self.h_pool_flat = tf.contrib.layers.batch_norm(tf.reshape(h_pool, [-1, self.num_filters * len(self.filter_sizes)]))
        
        if doc_embs:
            self.doc_embs2 = tf.layers.dense(self.doc_embs, 10, activation=tf.nn.relu, kernel_initializer = tf.initializers.he_uniform())
            self.h_pool_flat = tf.contrib.layers.batch_norm(tf.concat([self.h_pool_flat, self.doc_embs2], axis = 1))
    ...
    def train_on_batch(self, x, y, doc_embs):
        self.sess.run(self.optimizer, 
            feed_dict = {self.x : x, self.y : y, self.is_training : True, self.keep_prob : 0.2, self.doc_embs : doc_embs})

    def test_on_batch(self, x, doc_embs):
        return self.sess.run([self.predictions, self.h_pool_flat], 
